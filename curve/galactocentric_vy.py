...
def main():
    ...
    # bin_means, bin_edges, binnumber = stats.binned_statistic(df.x, df.vy / (-df.x / 1000), bins=200, range=[-16000, 0], statistic='median')
    # plt.stairs(bin_means, bin_edges, baseline=None)
    # plt.xlabel('Галактическая координата X, пк')
    # plt.ylabel('Галактическая угловая скорость Vy / R, км/с/кпк')
    # plt.title('Кривая вращения Галактики на луче зрения от Солнца к Центру')
    # plt.show()

    bin_means, bin_edges, binnumber = stats.binned_statistic(df.x, df.vy, bins=200, range=[-16000, 0],
                                                             statistic='median')
    plt.stairs(bin_means, bin_edges, baseline=None)
    plt.xlabel('Галактическая координата X, пк')
    plt.ylabel('Галактическая скорость Vy, км/с')
    plt.title('Скорость вращения Галактики на луче зрения от Солнца к Центру')
    plt.show()
